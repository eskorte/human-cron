...
cat $1 | while read line
	do 
		if [[ ! $line =~ ^\# && ! -z $line ]]; then
			...
			#command string
			ECHO_str="command: $COMMAND will run"
			...
			#process values
				...
				if [[ ! $HOUR =~ ^\*$ && ! -z $HOUR ]]; then
					if [[ $HOUR =~ ^\*\/[0-9]{1,2}$ && ! -z $HOUR ]]; then
						...
						HR_str="between hour $FR_NUM_HOUR and $TO_NUM_HOUR"				
						...
						HR_str="between hour $FR_NUM_HOUR and $TO_NUM_HOUR, every $NUM_HOUR hours"
					fi;
				else
					HR_str=""
				fi;
				...
				if [[ ! $MONTH_DAY =~ ^\*$ && ! -z $MONTH_DAY ]]; then
					if [[ $MONTH_DAY =~ ^\*\/[0-9]{1,2}$ && ! -z $MONTH_DAY ]]; then
						...
						MON_D_str="$MONTH_DAY of the month"
						...
						MON_D_str="between $FR_NUM_MONTH_DAY and $TO_NUM_MONTH_DAY of the month"						
						...
						MON_D_str="between $FR_NUM_MONTH_DAY and $TO_NUM_MONTH_DAY of the month, every $NUM_MONTH_DAY days"
					fi;						
				else
					MON_D_str=""
				fi;				
				...
				if [[ ! $MONTH =~ ^\*$ && ! -z $MONTH ]]; then
					if [[ $MONTH =~ ^\*\/[0-9]{1,2}$ && ! -z $MONTH ]]; then
						...
						MON_str="between $FR_NUM_MONTH and $TO_NUM_MONTH months, every $NUM_MONTH months"
					fi;						
				else
					MON_str=""
				fi;				
